<template>
  <q-layout class="non-selectable column no-wrap" view="hHh LpR lFr">
    ...
    <!-- Live preview of the media window, floating above the footer controls -->
    <MediaPreview />
    ...
    deleteCacheFiles('smart')
      .then(({ itemsDeleted }) => {
        if (itemsDeleted) {
          ...
          log('ℹ️ No cache items needed clearing', 'cacheAutoClear', 'info');
        }
      })
      .catch((error) => {
        errorCatcher(error);
...
async function handleAddWatchFolderEvent(
  ...
  log('🔍 [MainLayout] watchedItems:', 'mainLayout', 'log', watchedItems);
