...
## 결정 (2026-08-07): 전체 동기화 보류, 큰 버그만 선별

사용자 판단으로 **릴리스 단위 전체 동기화를 보류**하고, 앞으로도 **데이터 손실급 버그가 나올 때만 개별 반영**합니다.

### 재측정한 규모 (2026-08-07)

| 항목                            | 값                                           |
| ------------------------------- | -------------------------------------------- |
| 업스트림 커밋                   | 2,380 (비병합 1,839)                         |
| 변경 파일                       | 464                                          |
| **`v26.7.8` 직접 병합 시 충돌** | **90** — 내용 77 / 삭제-수정 8 / 추가-추가 5 |

> **아래 2026-08-05 절의 28 / 73 과 다른 이유**: 그 값은 당시 `origin/master` 기준이고 **틀린 값이 아닙니다.** 그 뒤 포크가 `jw-media.ts`, `mediaPlayback.ts`, `shared/vanilla.ts`, `src-electron/main/fs.ts` 등 업스트림 변경이 몰린 파일들을 계속 고쳤기 때문에 충돌이 73 → 90 으로 늘었습니다. **포크 작업을 계속하는 동안 이 숫자는 계속 커집니다.**
>
> 측정할 때 `git merge-tree --write-tree --name-only` 출력에는 파일 목록과 `Auto-merging`/`CONFLICT` 안내가 섞여 있으므로 **`^CONFLICT` 로 세야** 합니다. 파일 목록 줄만 세면 실제보다 크게, 앞쪽 몇 줄만 보면 작게 나옵니다.

### ⚠️ 동기화를 재개할 때 반드시 확인할 것 — 중국어가 조용히 사라집니다

업스트림이 **중국어 간체를 제거했습니다**(핀란드어·포르투갈(유럽)도). 번역 파일 삭제는 git 이 충돌로 잡아주지만 **등록 파일은 조용히 넘어갑니다**:

| 파일                       | 병합 결과                                 |
| -------------------------- | ----------------------------------------- |
| `src/i18n/cmn-hans.json`   | `CONFLICT (modify/delete)` — 눈에 보임 ✅ |
| `src/i18n/index.ts` (등록) | **충돌 없이 자동 병합** ⚠️                |

포크는 `index.ts` 에서 `import cmnHans from './cmn-hans.json'` 로 등록하는데 업스트림 `v26.7.8` 에는 그 등록이 없습니다(`cmn-hans` 가 타입 파일에만 남음). 자동 병합되면 **json 은 살아 있는데 언어 목록에서 빠집니다.** 충돌도 안 나고 빌드도 통과할 수 있습니다. 한국어는 삭제되지 않았습니다(내용 충돌만).

### 이번에 선별 반영한 것 (custom.10)

| 업스트림    | 내용                                              | 비고                                                                           |
| ----------- | ------------------------------------------------- | ------------------------------------------------------------------------------ |
| `75eb3c09f` | 권한 오류로 `.jwpub` 삭제 + Windows `EEXIST` 경쟁 | **그대로 이식 불가** — 의존 함수 4개가 포크에 없고 코드 위치도 다름. 적응 필요 |
| `a31362be2` | 시작 시 미인식 회중 폴더 삭제                     | 호출부 2곳 연결                                                                |

### 검토 후 반영하지 않은 것

| 업스트림                              | 이유                                                                            |
| ------------------------------------- | ------------------------------------------------------------------------------- |
| `c294c71b1`, `657ad1cd3`, `91e0523d7` | macOS security-scoped bookmark — macOS 빌드만 해당                              |
| `2ca62199f`                           | icongenie → sharp/png2icons (취약 전이 의존성 제거) — 빌드 시점만, 앱 동작 무관 |
| `88d186d5c`                           | 일정 동기화 후 미디어 갱신 — 관련 있으나 무관한 변경과 묶인 PR                  |
| `3e3df6b38`                           | 손상된 `.last-used` 파싱 — 캐시 정리 시점만 영향, 경미                          |
| `ed6b77c3a`                           | 손상된 jw-icons 폰트 복구 — 포크는 자체 폰트 마이그레이션 사용                  |

### 다음에 갭을 다시 볼 때

`git merge-tree --write-tree --name-only HEAD <tag> | grep '^CONFLICT'` 로 세십시오. 그리고 **개별 이식은 문맥을 잃습니다** — 이번 `75eb3c09f` 처럼 의존 함수가 없을 수 있고, 2026-08-06 에는 커밋 하나를 떼어 옮긴 것이 회귀 두 건을 만들었습니다([congregation-lookup-broken.md](./congregation-lookup-broken.md)).
