...
export interface MediaPlayingState {
  ...
  /**
   * `Date.now()` timestamp at which `currentPosition` was last set from a media
   * window report. Lets consumers extrapolate the real, current position
   * (accounting for report throttling/round-trip time) instead of treating
   * `currentPosition` as still accurate by the time they read it.
   */
  currentPositionUpdatedAt: number;
  pan: Partial<{ x: number; y: number }>;
  /**
   * Set to the current `playToken` once `currentPosition` is observed actually
   * advancing for that play request, not just reporting a value. Lets consumers
   * like the media preview wait for confirmed, moving playback instead of
