...
2. **집회 요일이 하루씩 밀려 저장됐습니다.** (v26.4.7-custom.5 에서 수정)
   ...
   **증상**: 일요일 주말집회가 월요일로 저장 → **주말집회 섹션이 월요일에 생성**되고 실제 주말집회 날짜에는 아무것도 없습니다. 미디어 가져오기와 사전집회 시계(`useTimer` 가 `weDay` 를 읽음)도 함께 어긋납니다.

   > ⚠️ **함께 보고된 "공개강연 연제 카드가 사라짐"은 이 버그가 원인이 아니었습니다.** 조사 중 두 증상을 인과로 이었지만, 확인해 보니 해당 프로필의 `enablePublicTalkTitle` 이 `false` 였습니다. 그 설정이 꺼져 있으면 요일과 무관하게 카드가 표시되지 않습니다([`MediaList.vue:37`](../src/components/media/MediaList.vue#L37)). 사용자가 직접 지적해 바로잡았습니다.
   >
   > 요일 밀림이 실제 주말집회 날짜에서 카드를 가리는 것도 **사실이지만**, 이번 사례의 실제 원인은 설정이었습니다. 설정이 원래 켜져 있었다는 증거 없이 사슬로 이은 것이 잘못입니다.
