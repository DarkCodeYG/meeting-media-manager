...
### ⚠️ 미해결 관찰 — 추출된 자막이 사라짐

추출 산출물은 `getTempPath()`(`getCachePath('Temp', true)`)에 씁니다. 이 조사 중 **`Temp` 디렉터리 전체가 사라진 것을 확인**했습니다 — 11시경에는 VTT 가 있었고 12시경에는 디렉터리 자체가 없었습니다. 그 사이 저장된 `subtitlesUrl` 은 없는 파일을 가리키게 되므로 **자막이 조용히 다시 안 나오게 됩니다.**

단, 원인을 특정하지 못했습니다:

- [`cleanup.ts:92`](../src/helpers/cleanup.ts#L92) 는 `subtitlesUrl` 을 **참조 목록에 포함**하므로 스마트 캐시 정리는 이 파일을 보호합니다
- [`cleanup.ts:520`](../src/helpers/cleanup.ts#L520) 은 `Temp` 를 `fetchUntouchableDirectories` 에 넣어 **디렉터리 자체를 삭제 대상에서 제외**합니다

즉 정상 경로로는 설명되지 않습니다. 전체 캐시 삭제, 사용자의 수동 삭제, 앱 시작 시 정리 중 무엇인지 확인해야 합니다. **원인을 모르는 상태에서 저장 위치를 바꾸지 않았습니다.** 다음 조사 착수점: `getCachePath` 의 두 번째 인자(`true`)가 무엇을 뜻하는지, 그리고 전체 캐시 삭제 경로가 `fetchUntouchableDirectories` 를 존중하는지.
