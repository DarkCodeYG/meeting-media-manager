...
업스트림에서 **데이터가 지워지는 버그 두 건**만 가져왔습니다. 포크는 당분간 현재 기준 버전을 유지하지만, 이 둘은 되돌리기 어려운 파일이 삭제되므로 먼저 반영했습니다.

- 🐞 **읽을 수 없는 발행물을 삭제하던 문제**: `.jwpub` 을 열거나 압축을 풀 때 **권한 오류가 나면 파일이 손상된 것과 똑같이 처리해 이미 내려받은 발행물을 삭제**했습니다. 그리고 다시 내려받아도 같은 폴더에 접근할 수 없어 반복됐습니다. 네트워크 공유 폴더, 다른 프로그램이 잠근 폴더, 재연결 중인 드라이브 — 이런 경우 파일이 헛되이 사라졌습니다. 이제 **손상이 아니라 접근 문제일 때는 발행물을 그대로 둡니다**(진짜 손상된 파일은 여전히 다시 받습니다). Windows 에서 압축 해제 두 개가 같은 폴더를 동시에 만들려 할 때 하나가 실패하던 문제도 함께 고쳤습니다.

- 🐞 **앱을 켤 때마다 모르는 회중의 저장된 자료를 삭제하던 문제**: 시작할 때 **이 앱이 알지 못하는 회중 ID의 추가 미디어·회중 설정·공개 강연 개요를 모두 삭제**했습니다. 회중 ID 는 설치본마다 따로 만들어지므로, **캐시 폴더를 공유하는 설치본이 둘 이상이면**(그 용도로 **캐시 폴더** 설정이 있습니다) 서로의 자료를 "모르는 것"이라는 이유로 지웠습니다. 이제 **사용자가 직접 회중을 삭제할 때만** 저장된 자료를 지웁니다.

## v26.4.7-custom.9

### 🐞 버그 수정

- 🐞 **자막을 켜도 이미 추가한 미디어에는 적용되지 않던 문제**: 각 항목의 자막 파일은 **미디어를 추가하거나 가져올 때 한 번 정해지고 다시 검토되지 않았습니다.** 그래서 자막 설정이 꺼진 상태에서 추가한 영상은 **몇 번을 다시 재생해도 자막이 나오지 않고**, 삭제하고 다시 추가하는 것이 유일한 방법이었습니다. 특히 앱을 새로 설치하면 자막 설정이 꺼진 상태로 시작하므로, 그 설정을 찾기 전에 수집된 미디어가 전부 이 상태가 됩니다.

  이제 **설정을 켜면 이미 저장된 미디어를 다시 훑고**, 그래도 자막이 없는 항목은 **재생할 때 백그라운드에서 찾아** 다음 재생부터 나옵니다. 자동 수집된 집회 미디어는 자막을 다시 찾는 데 필요한 정보가 항목에 남아 있지 않아 **다시 가져옵니다** — 직접 추가하신 미디어는 그대로 유지됩니다.
