...
- **상태**: 6건 수정 완료 / **1건 미해결(원인 미상, 사용자 판단으로 중단)**
  - 수동 추가 영상에 자막 미설정 — `100878ef9` (custom.1)
  - `.jwpub` 영상의 `FilePath` 관문 — custom.6
  - mediator API 에 없는 발행물 — custom.7
  - 추출 완료 전 재생 시 스냅샷 고정 — custom.7
  - 출처별 자막 위치 불일치 — custom.8
  - **설정을 켜도 기존 미디어에 적용 안 됨 — custom.9** (사용자가 동작만 보고 원인을 정확히 추론)
  - ⚠️ **《용기》 책 부가 영상 하나만 자막이 바닥에 붙음 — 원인 미상** (아래)
- **작성일**: 2026-08-05 / **최종 갱신**: 2026-08-07
...
## 0.00 자막 설정을 켜도 기존 미디어에 적용되지 않음 (custom.9)

**증상**: 자막 설정이 꺼진 상태에서 추가한 영상은 나중에 설정을 켜도 **몇 번을 다시 재생해도 자막이 나오지 않음.** 삭제하고 다시 추가하는 것만이 방법.

**사용자가 동작만 보고 원인을 정확히 추론했습니다.** 코드가 그대로였습니다 — `subtitlesUrl` 을 설정하는 곳은 두 군데뿐이고([`jw-media.ts:380`](../src/helpers/jw-media.ts#L380) 수동 추가, [`jw-media.ts:1908`](../src/helpers/jw-media.ts#L1908) 발행물 미디어) **둘 다 추가·수집 시점**입니다. `enableSubtitles` 변경을 감지해 다시 해석하는 코드가 **어디에도 없었습니다.**

신규 설치 시 `enableSubtitles` 기본값이 `false` 이므로(`settings.ts`), 설정을 찾기 전에 수집된 미디어가 **전부** 이 상태가 됩니다.

**수정 (두 경로)**

| 시점         | 동작                                                                          |
| ------------ | ----------------------------------------------------------------------------- |
| 설정을 켤 때 | `backfillMissingSubtitles()` — 저장된 미디어를 훑음                           |
| 재생할 때    | `resolveSubtitlesIfMissing()` — 빈 항목을 백그라운드 해석, 다음 재생부터 적용 |

**제약**: 발행물 미디어는 제자리 재해석이 불가능합니다. `getSubtitlesUrl` 은 `KeySymbol`+`Track` 으로 조회하는데 `MediaItem` 에 둘 다 없습니다(`pubMediaId` 는 빈 값을 걸러 join 한 문자열이라 역파싱 불가). 그래서 `updateLookupPeriod({ reset: true })` 로 재수집합니다.

**배선 전에 안전성을 확인했습니다** — `resetDay` 가 `source !== 'dynamic'` 으로 필터링하므로 수동 추가 미디어는 보존됩니다. 설정 토글이 미디어를 지울 수 있다면 버그보다 훨씬 나쁘기 때문입니다.

## 0.0 출처에 따라 자막 위치가 달라짐 (custom.8)

**증상**: 자막은 나오지만 **영상 출처에 따라 위치가 다름.**

**원인**: 실제 파일 두 개를 비교하면 그대로 보입니다.

```
# JW 공식 자막 (다운로드) — 매 큐에 배치 설정이 있음
00:00:01.105 --> 00:00:02.231 line:90% position:50% align:center

# ffmpeg 추출 자막 (내장 mov_text 에서) — 설정 없음
00:01.480 --> 00:03.065
```

JW 자막은 `line:90%` 로 **바닥에서 살짝 위**에 놓이고, 앱이 직접 만든 자막(추출 또는 SRT 변환)은 설정이 없어 플레이어 기본 위치인 맨 아래에 붙습니다. 같은 집회 안에서 미디어 출처가 섞이면 자막이 위아래로 튑니다.

**`::cue` CSS 로는 고칠 수 없습니다.** 위치는 WebVTT 큐 설정이지 CSS 속성이 아니라서, `::cue` 로는 색·글꼴만 지정할 수 있습니다. **VTT 를 만들 때 설정을 넣어야** 합니다.

**수정**: [`src/shared/vanilla.ts`](../src/shared/vanilla.ts) 의 `applyDefaultCuePlacement()` 가 **설정이 없는 큐에만** JW 와 같은 `line:90% position:50% align:center` 를 붙입니다. 이미 설정이 있는 큐는 건드리지 않습니다 — 그 값은 자막 제작자의 의도일 가능성이 높습니다(화면의 다른 글자를 피하려고 위로 올린 경우 등).

두 생성 경로에 모두 적용했습니다. 한쪽이 렌더러(SRT 사이드카 변환), 다른 쪽이 메인 프로세스(FFmpeg 추출)라 공용 위치가 필요합니다.

> 처음에는 `src/shared/subtitles.ts` 라는 새 파일에 두었는데 **`src-electron/main/__tests__/architecture.test.ts` 가 이를 거부했습니다.** 메인 프로세스는 `src/types`, `src/constants`, 그리고 이름에 `vanilla` 가 포함된 파일에서만 import 할 수 있습니다. `vanilla.ts` 가 프로세스 무관 공용 코드의 의도된 위치이므로 거기로 옮겼습니다 — 규칙을 완화해 새 파일을 허용 목록에 넣는 쪽이 아니라. 아키텍처 테스트가 제 역할을 했습니다.

| 경로               | 위치                                                                                    |
| ------------------ | --------------------------------------------------------------------------------------- |
| SRT 사이드카 → VTT | `srtToVtt()` in [`helpers/fs.ts`](../src/helpers/fs.ts)                                 |
| 내장 자막 추출     | `extractSubtitles()` in [`src-electron/main/ffmpeg.ts`](../src-electron/main/ffmpeg.ts) |

**테스트**: `src/shared/__tests__/subtitles.test.ts` 7개. 두 출처의 **실제 타임스탬프 형식**(`MM:SS.mmm` / `HH:MM:SS.mmm`)을 모두 다루고, JW 자막을 넣으면 **바이트 단위로 그대로 반환**되는지 검증합니다.

### 사용자 확인 완료 (2026-08-07)

세 가지 출처 모두 위치가 적절함을 확인했습니다:

| 영상                                   | 자막 출처                                  |
| -------------------------------------- | ------------------------------------------ |
| `中央长老团维护团结`                   | 라이브러리 자동 수집 미디어의 JW `.vtt`    |
| `你愿意为好消息做什么？` (`S-341-26v`) | `.jwpub` → JW `.vtt`                       |
| `1112024059_CHS_cnt_1_r720P.mp4`       | 내장 자막 추출 + 이 수정이 `line:90%` 부여 |

### 미해결 1건 — 특정 영상 하나만 자막이 바닥에 붙음

**대상**: 2026-08-13 목요일 **그리스도인 생활**, **《용기》 책 부가 영상** (`你从他们身上学到什么——撒拉`). 다른 영상들은 전부 정상이고 이 영상에서만 자막이 바닥 라인에 붙습니다.

**확인된 것**

- 자막 버튼으로 켜고 꺼짐 → **하드섭이 아니라 실제 `<track>`** 입니다
- 앱은 JW 자막 파일을 수정하지 않습니다(다운로드해 그대로 재생)

**배제된 것**

JW 자막에 두 가지 큐 형식이 존재하는 것은 사실입니다:

```
형식 A  line:90% position:50% align:center           (jwb-089, jwb-098, jwb-102, jwb-106, S-341-26v)
형식 B  line:90%,end position:50%,center align:center (jwb_CHS_202103_03, jwbcov21_CHS_10)
```

한때 이 차이가 원인이라고 보았지만 **반증되었습니다** — 사용자가 두 형식의 영상을 각각 확인한 결과 **양쪽 다 위치가 정상**입니다(`S-341-26v` = A, `jwb_CHS_202103_03` = B). 따라서 큐 형식은 이 증상을 설명하지 않습니다.

**구하지 못한 것**

해당 영상의 `.vtt` 파일입니다. 그 미디어는 스트리밍되어 디스크에 저장되지 않았고, 프로필 스토어 스냅샷에도 그 항목이 없었습니다(해당 날짜에 노래만 들어 있음). 파일 없이는 확정할 수 없습니다.

**다음 조사자에게**

1. 8/13 미디어를 **다운로드**시켜 `.vtt` 를 디스크에 남기고 큐 설정을 직접 읽으십시오
2. 그 영상이 **`.jwpub` 발행물(《용기》) 경유**라는 점이 다른 영상들과의 유일한 차이입니다 — 발행물 DB 의 `Multimedia` 행이 다른 필드 조합을 갖고 있을 수 있습니다
3. **추측으로 고치지 마십시오.** 이 항목을 조사하며 큐 형식 가설을 세웠다가 반증되었고, 그 전에 `line:90%` 렌더링 메커니즘도 잘못 단정했습니다(아래 철회 절)

**사용자 판단으로 여기서 중단했습니다** — 다른 모든 영상이 정상이고 이 한 건은 실사용에 큰 지장이 없습니다.

### ⚠️ 조사 중 제가 만들어낸 틀린 설명 (철회)

조사 도중 "`line:90%` 는 퍼센트 값이라 snap-to-lines 를 끄고 자막 상자의 **위쪽 끝**을 90% 지점에 고정하므로, 텍스트가 아래로 뻗어 바닥을 넘는다"고 단정했습니다. **틀렸습니다.**

`S-341-26v_CHS_01.vtt` 를 직접 내려받아 보니 `line:90%` 가 들어 있고, 그 영상이 바로 사용자가 "위치 적당함"이라고 확인한 것입니다. 스펙 문장을 해석해 메커니즘을 만들어냈고 **화면으로 확인하지 않았습니다.**

그 잘못된 전제로 "모든 자막에서 배치 설정을 제거해 플레이어 기본 위치로 통일"하는 변경을 만들다가, 사용자 확인으로 반증되어 **커밋 전에 폐기**했습니다. **잘 동작하는 것을 되돌리는 변경이었습니다.**

원래 이 수정을 넣은 근거 — "앱의 주된 미디어 출처가 쓰는 배치에 맞춘다" — 가 맞는 근거였습니다. 나중에 덧붙인 메커니즘 이야기가 불필요했고 틀렸습니다.

### 취소된 관찰 — 추출된 자막이 사라진 것은 버그가 아니었습니다

조사 중 `Temp` 디렉터리가 사라진 것을 보고 "저장된 `subtitlesUrl` 이 없는 파일을 가리키게 되는 버그"로 적었습니다. **사용자가 앱을 삭제한 것이었습니다.** 앱 데이터 루트(`%APPDATA%\Meeting Media Manager`)와 설치본이 모두 없는 것을 확인해 확정했습니다.

코드 쪽 근거도 처음부터 이것을 가리키고 있었습니다 — 정상 경로로는 설명되지 않는다고 적어놓고도 외부 원인을 확인하지 않았습니다:

- [`cleanup.ts:92`](../src/helpers/cleanup.ts#L92) 는 `subtitlesUrl` 을 참조 목록에 포함하므로 스마트 캐시 정리는 이 파일을 **보호합니다**
- [`cleanup.ts:520`](../src/helpers/cleanup.ts#L520) 은 `Temp` 를 `fetchUntouchableDirectories` 에 넣어 디렉터리 자체를 **삭제 대상에서 제외합니다**

**교훈**: 파일이 사라졌을 때 앱 내부 경로만 뒤졌습니다. 사용자의 조작(삭제·재설치)이 훨씬 단순한 설명인데 확인하지 않았고, 그 결과 실재하지 않는 버그를 문서와 커밋 메시지(`0229c3e85`)에 남겼습니다. 커밋 메시지는 그대로 남아 있으니 이 절이 정정입니다.

`getTempPath()` 저장 위치는 그대로 둡니다 — 바꿀 이유가 없었습니다.

## 문서 구성

| 절   | 내용                                                                  | 상태          |
| ---- | --------------------------------------------------------------------- | ------------- |
| 0.00 | 설정을 켜도 기존 미디어에 적용되지 않음                               | custom.9 수정 |
| 0.0  | 출처에 따라 자막 위치가 달라짐 (+ 미해결 1건)                         | custom.8 수정 |
...
### 진단은 확증됨, 수정은 효과 없음 (2026-08-07, 미해결)

**타이밍이 원인이라는 진단은 사용자 확인으로 확증되었습니다.** 같은 파일·같은 설정에서 추가 직후 재생 시 자막이 없었고, **앱을 껐다 켜고 다시 재생하니 나왔습니다.** 스토어에는 URL 이 있었으므로 남은 변수는 재생 시점 스냅샷뿐입니다.

같은 관찰이 두 출처의 차이도 설명합니다 — `.jwpub` 영상은 자막이 **수 KB 다운로드**라 재생 버튼을 누를 때 이미 준비되어 있고, 166 MB 영상의 **FFmpeg 추출은 수 분** 걸립니다. 목적지는 같고 지연만 다릅니다.

**그러나 이 절의 수정은 의도한 효과를 내지 못했습니다.** 재시작 없이 재생 중에 나타나게 하려던 것인데 재시작이 필요했습니다. 즉 **재생 중 전달이 작동하지 않습니다.**

남은 원인 후보는 아래 미확인 항목입니다:

> `<track>` 에 `default` 속성이 있지만([`MediaPlayerPage.vue:122-127`](../src/pages/MediaPlayerPage.vue#L122-L127)) **재생 중에 추가된 트랙을 Chromium 이 활성화하지 않는 것으로 보입니다.** `default` 는 리소스 선택 시점에 적용되는 속성입니다. 고치려면 미디어 창에서 트랙이 도착할 때 `textTracks[0].mode = 'showing'` 을 명시해야 합니다. 코드베이스 전체에 `textTracks` 사용처가 없으므로 신규 작업입니다.

**현재 사용자에게 유효한 안내**: 긴 영상을 추가한 직후 바로 재생하면 자막이 없을 수 있습니다. **잠시 뒤 다시 재생**하면 나옵니다.
