 ...
 * `Additional Media/{congId}`. Those folders aren't tied to a fetchable fileUrl
 * the way jw.org media is, so they can't rely on the in-memory store to know
 * they're still needed — a cache clear racing the background meeting-fetch queue
 * would see them as unreferenced and delete media the user added by hand.
 * Protect any dated subfolder whose date is today or in the future, regardless
 * of what is currently loaded in the store.
 */
...
async function processCacheDirectory(
  ...
  try {
    ...
    for (const item of items) {
      ...
      // Protect additional media saved for a future/today's meeting, even if the
      // store hasn't (re)loaded that day's media yet this session
      if (!isReferenced) {
